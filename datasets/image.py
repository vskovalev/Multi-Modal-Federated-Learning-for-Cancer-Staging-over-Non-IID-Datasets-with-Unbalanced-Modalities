...
from dataset_utils import remove_unwanted_labels, map_to_one_hot
from datasets.dataset_classes import CustomImageDatasetStratified

def build_img_train_test_strat(fraction, random_state=None):
    if random_state:
        random.seed(random_state)
    
    final_df = pd.read_csv('Complete_image_plus_stage_balanced.csv', delimiter=",")
    ...
    train_dataset = CustomImageDatasetStratified(first_set_inputs, first_set_labels)
    test_dataset = CustomImageDatasetStratified(second_set_inputs, second_set_labels)
