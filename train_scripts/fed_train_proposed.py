...
from fed_utils import average_encoder_weights, modality_to_classifier_mapper, classifier_to_modality_mapper
from fed_arg_parser import parse_arguments
from set_up_save_dir import set_up_base_fed
...
def main():
    ...
    if args.acc_used:
        if torch.cuda.is_available():
            logging.info("choosing cuda gpu")
            device = torch.device("cuda:0")
        elif torch.backends.mps.is_available():
            logging.info("choosing mps gpu")
            device = torch.device("mps")
        else:
            logging.info("accelerator not found - falling back to cpu")
    ...
    for cohort in COHORTS:
        ...
        for clientbuildnum in range(len(datasets)):
            if clientbuildnum==0:
                ...
                client['model'] = CustomFederatedModel(client['dataset'].modalities, client['dataset'].column_map)
                if args.acc_used:
    ...
    network['global_model'] = CustomFederatedModel(modalities=["mrna", "image"], column_map=network['validation_datasets'][0].column_map)
    ...
    network['global_model'].load_state_dict(torch.load(os.path.join(args.saved_model_path, 'federated_mrna_image_start_model.pt')))
    ...
    for classifier_pair in network['global_classifiers'].keys():
        ...
        dummy_model = CustomFederatedModel(modalities=classifier_to_modality_mapper(classifier_pair), column_map=network['validation_datasets'][0].column_map)
